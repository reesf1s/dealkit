...
export type CrmViewFilters = z.infer<typeof viewFiltersSchema>
...
function eventToView(event: { id: string; metadata: unknown; createdAt: Date }): CrmSavedViewDto | null {
  ...
  return {
    id: event.id,
    name: data.name,
    description: typeof data.description === 'string' ? data.description : 'Workspace saved view.',
    scope: 'workspace',
...
export function getSystemCrmViews() {
  return systemViews
}
...
export async function getWorkspaceCrmViews(workspaceId: string): Promise<CrmSavedViewDto[]> {
  const rows = await db
    .select({ id: events.id, metadata: events.metadata, createdAt: events.createdAt })
    .from(events)
...
export function viewMatchesLead(view: CrmSavedViewDto, lead: CrmLeadDto) {
