  ...
  return [
    {
      ...
      stageName: "Discovery",
      description:
        "Requested custom plan for 40-seat rollout. CTO asked for implementation timeline and deployment notes.",
    ...
    },
    {
      ...
      stageName: "Proposal",
      description:
        "LinkedIn Sales Navigator lead. Wants pricing proof, implementation references, and rollout confidence.",
    ...
    },
    {
      ...
      stageName: "Follow-up",
      description:
        "Lead came from a shared spreadsheet import. Email went cold after two unanswered outreach attempts.",
    ...
    },
    {
      ...
      description:
        "LinkedIn message from CFO. Budget available and implementation window is two weeks.",
...
function seededMessages(
  ...
  return [
    {
      workspaceId,
      leadId: byTitle.get("Harborline Studio")!.id,
      channel: "mail",
      fromRole: "customer",
      body: "Can we talk before Friday? We need numbers by end of week.",
      sentAt: date(-1),
    },
    {
      workspaceId,
      ...
      channel: "mail",
      fromRole: "rep",
      body: "Absolutely. I can share a 2-step rollout and pricing breakdown today.",
      sentAt: date(-1),
    },
    {
      workspaceId,
      leadId: byTitle.get("Northway Retail")!.id,
      channel: "linkedin",
      fromRole: "customer",
      body: "I liked your one-pager. What kind of onboarding support is included?",
      sentAt: date(-2),
    },
    {
      workspaceId,
      ...
      channel: "linkedin",
      fromRole: "rep",
      body: "I can cover implementation planning and first-week training in every plan.",
      sentAt: date(-2),
    },
    {
      workspaceId,
      leadId: byTitle.get("Orbit Logistics")!.id,
      channel: "linkedin",
      fromRole: "customer",
      body: "Need budget-aligned option for 45 seats. Can we discuss Monday?",
      sentAt: date(-3),
    },
    {
      workspaceId,
      leadId: byTitle.get("Crescent Systems")!.id,
      channel: "webchat",
      fromRole: "customer",
      body: "Hi there, are you available for a quick demo?",
      sentAt: date(-8),
    },
    {
      workspaceId,
      ...
      channel: "webchat",
      fromRole: "rep",
      body: "Yes, I can book a 20-min discovery window for tomorrow.",
      sentAt: date(-8),
    },
  ];
}
  ...
  return [
    {
      ...
      description:
        "Build a three-step onboarding checklist for Harborline Studio.",
      priority: "high",
      dueAt: date(3),
      companyName: "Harborline Studio",
    ...
    },
    {
      workspaceId,
      leadId: byTitle.get("Northway Retail")!.id,
      title: "Pull case study pack",
      description: "Collect references for SME implementation with 30+ users.",
      priority: "medium",
      dueAt: date(5),
      companyName: "Northway Retail",
      personName: "Jules Moreno",
    },
    {
      workspaceId,
      leadId: byTitle.get("Orbit Logistics")!.id,
      title: "Send revised commercial model",
      description:
        "Draft a 12-week plan with rollout costs and success checkpoints.",
      priority: "high",
      dueAt: date(6),
      companyName: "Orbit Logistics",
      personName: "Sofie Grant",
    },
  ];
}
  ...
  return [
    {
      ...
      leadId: byTitle.get("Harborline Studio")!.id,
      title: "Email opened",
      body: "Recipient viewed outreach after three reminders.",
      type: "engagement",
      occurredAt: date(-1),
      companyName: "Harborline Studio",
      personName: "Maya Chen",
    },
    {
      ...
      body: "Customer asked for deployment timeline and support availability.",
      type: "reply",
      occurredAt: date(-2),
      companyName: "Northway Retail",
      personName: "Jules Moreno",
    },
    {
      workspaceId,
      leadId: byTitle.get("Crescent Systems")!.id,
      title: "Webchat bounce",
      body: "No reply after first message, lead still warm based on visit frequency.",
      type: "follow_up",
      occurredAt: date(-5),
      companyName: "Crescent Systems",
      personName: "Ari Patel",
    },
  ];
}
...
export async function ensureCrmSeeded(workspaceId: string, ownerId: string) {
  ...
  await db
    .insert(crmChannels)
    .values(seedChannels.map((channel) => ({ workspaceId, ...channel })))
    .onConflictDoNothing();
  const insertedLeads = await db
    .insert(crmLeads)
    .values(leadSeed(workspaceId, ownerId))
    .returning();
  await db
    .insert(crmMessages)
    .values(seededMessages(workspaceId, insertedLeads));
  await db.insert(crmTasks).values(seededTasks(workspaceId, insertedLeads));
  await db
    .insert(crmActivities)
    .values(seededActivities(workspaceId, insertedLeads));
}
...
function leadToDto(lead: CrmLeadRow): CrmLeadDto {
  return {
    id: lead.id,
    title: lead.title,
...
function messageToDto(message: CrmMessageRow): CrmMessageDto {
  return {
    ...
    text: message.body,
    time: message.sentAt.toLocaleTimeString("en-GB", {
      hour: "2-digit",
      minute: "2-digit",
    }),
...
export async function getCrmWorkspacePayload(
  workspaceId: string,
  ...
  const [leads, channels, messages, tasks, activities] = await Promise.all([
    db
      ...
      .from(crmLeads)
      .where(eq(crmLeads.workspaceId, workspaceId))
      .orderBy(desc(crmLeads.score), desc(crmLeads.updatedAt)),
    db
      .select()
      .from(crmChannels)
      .where(eq(crmChannels.workspaceId, workspaceId))
      .orderBy(asc(crmChannels.name)),
    db
      .select()
      .from(crmMessages)
      .where(eq(crmMessages.workspaceId, workspaceId))
      .orderBy(asc(crmMessages.sentAt)),
    db
      .select()
      .from(crmTasks)
  ...
  const [lead] = await db
    ...
    .where(
      and(
        ...
        eq(crmLeads.workspaceId, input.workspaceId),
      ),
    )
    .returning();
...
function cleanLeadInput(input: LeadMutationInput) {
  const probability = Number.isFinite(input.probability)
    ? Math.max(0, Math.min(100, Math.round(input.probability!)))
    : undefined;
  const valueAmount = Number.isFinite(input.valueAmount)
    ? Math.max(0, Math.round(input.valueAmount!))
    : undefined;
  ...
  return {
    ...
    status:
      ...
      input.status === "discovery" ||
      input.status === "qualification" ||
      input.status === "won" ||
      input.status === "lost"
        ? input.status
        : undefined,
...
function createDemoCrmLead(data: LeadMutationInput): CrmLeadDto {
  ...
  const nextStep =
    cleaned.nextStep || "Qualify the opportunity and confirm the next step.";
  ...
  return {
    ...
    description:
      cleaned.description || "New opportunity created from the CRM canvas.",
    nextStep,
    companyName,
...
function createDemoCrmMessage(input: {
  leadId: string;
  channel: ChannelId;
  ...
  return {
    ...
    from: input.from ?? "rep",
    text: input.text,
    time: sentAt.toLocaleTimeString("en-GB", {
      hour: "2-digit",
      minute: "2-digit",
    }),
...
function createDemoCrmTask(input: TaskMutationInput): RecoveryTask {
  ...
  return {
    ...
    description:
      cleaned.description ||
      "Complete the next sales action and update the deal record.",
...
function createDemoCrmActivity(input: ActivityMutationInput): RecoveryActivity {
  ...
  return {
    ...
    body:
      cleaned.body ||
      "Captured a customer interaction and linked it back to the active deal.",
    type: cleaned.type,
...
function setDemoChannelConnection(
  provider: ChannelId,
  connected: boolean,
): CrmChannelDto {
  const existing = seedChannels.find(
...
export async function createCrmLead(input: {
  ...
  const nextStep =
    data.nextStep || "Qualify the opportunity and confirm the next step.";
  ...
  const [lead] = await db
    ...
    .values({
      ...
      description:
        data.description || "New opportunity created from the CRM canvas.",
      nextStep,
      companyName,
      primaryPersonName: personName,
...
export async function completeCrmTask(input: {
  workspaceId: string;
  taskId: string;
}) {
  const [task] = await db
    ...
    .where(
      and(
        ...
        eq(crmTasks.workspaceId, input.workspaceId),
      ),
    )
    .limit(1);
  ...
  if (task.leadId) {
    ...
    if (lead) {
      await db
        .update(crmLeads)
        .set({
          openTaskCount: Math.max(0, lead.openTaskCount - 1),
          updatedAt: new Date(),
        })
        .where(
          and(
...
export async function deleteCrmActivity(input: {
  workspaceId: string;
  activityId: string;
}) {
  const [activity] = await db
    ...
    .where(
      and(
        ...
        eq(crmActivities.workspaceId, input.workspaceId),
      ),
    )
    .returning();
...
export async function createCrmMessage(input: {
  workspaceId: string;
  leadId: string;
  channel: ChannelId;
  from: CrmMessageDto["from"];
...
export async function setChannelConnection(input: {
  workspaceId: string;
  provider: ChannelId;
  connected: boolean;
}) {
  const [channel] = await db
    ...
    .values({
      ...
      name:
        seedChannels.find((channel) => channel.provider === input.provider)
          ?.name ?? input.provider,
...
export function buildFallbackDraft(
  lead: CrmLeadDto,
  intent = classifyRecoveryIntent(lead),
) {
  const action =
  ...
  const nextStep = (lead.nextStep || "scope, timeline, and next actions")
    .trim()
    .replace(/[.!?]+$/, "");
...
export async function recordAiDraft(input: {
  workspaceId: string;
  leadId: string;
  prompt: string;
  result: string;
...
export function getDemoCrmLead(leadId: string) {
  return (
    getDemoCrmWorkspaceState().leads.find((lead) => lead.id === leadId) ?? null
  );
}
...
export function addDemoCrmMessage(input: {
  leadId: string;
  channel: ChannelId;
  from?: CrmMessageDto["from"];
  text: string;
  ...
  const lead = workspace.leads.find(
    (candidate) => candidate.id === input.leadId,
  );
  ...
  workspace.messages = {
    ...workspace.messages,
    [message.channel]: [
      ...(workspace.messages[message.channel] ?? []),
      message,
    ],
  };
  workspace.leads = workspace.leads.map((candidate) =>
    candidate.id === input.leadId
      ? {
          ...candidate,
          latestActivityAt: message.sentAt,
...
export function updateDemoCrmLead(input: {
  leadId: string;
  data: LeadMutationInput;
}) {
  const workspace = getDemoCrmWorkspaceState();
  ...
  const updated: CrmLeadDto = {
    ...
    probability,
    expectedCloseDate: cleaned.expectedCloseDate
      ? cleaned.expectedCloseDate.toISOString()
  ...
  workspace.leads = workspace.leads.map((candidate) =>
    candidate.id === input.leadId ? updated : candidate,
  );
  refreshDemoWorkspace(workspace);
  return updated;
}
...
export function deleteDemoCrmLead(leadId: string) {
  ...
  workspace.leads = workspace.leads.filter(
  ...
  if (lead) {
    workspace.leads = workspace.leads.map((candidate) =>
      candidate.id === lead.id
        ? {
            ...candidate,
            openTaskCount: Number(candidate.openTaskCount ?? 0) + 1,
          }
        : candidate,
    );
  }
      ...
      candidate.companyName === task.companyName
        ? {
            ...
            openTaskCount: Math.max(
              0,
              Number(candidate.openTaskCount ?? 0) - 1,
            ),
          }
        : candidate,
    );
...
export function addDemoCrmActivity(input: ActivityMutationInput) {
  ...
  if (lead) {
    workspace.leads = workspace.leads.map((candidate) =>
      candidate.id === lead.id
        ? { ...candidate, latestActivityAt: activity.occurredAt }
        : candidate,
    );
  }
  refreshDemoWorkspace(workspace);
...
export function updateDemoChannelConnection(
  provider: ChannelId,
  connected: boolean,
) {
  const workspace = getDemoCrmWorkspaceState();
  ...
  workspace.channels = exists
    ? workspace.channels.map((candidate) =>
        candidate.id === provider ? channel : candidate,
      )
...
export function getDemoCrmWorkspacePayload(): CrmWorkspacePayload {
  const leads: CrmLeadDto[] = [
    {
      ...
      description:
        "Owner is comparing Halvex against Gong for call intelligence. Budget is available this month, but legal needs security notes before Friday.",
      nextStep:
        "Book decision call with owner and send security notes plus Gong comparison.",
  ...
  const messages: Record<ChannelId, CrmMessageDto[]> = {
    mail: [
      demoMessage({
        id: "demo-msg-1",
        leadId: "demo-harborline",
        channel: "mail",
        from: "customer",
        text: "Can we talk before Friday? We need numbers by end of week.",
        daysAgo: 1,
      }),
      demoMessage({
        id: "demo-msg-2",
        ...
        channel: "mail",
        from: "rep",
        text: "Absolutely. I can share a 2-step rollout and pricing breakdown today.",
        daysAgo: 1,
      }),
    ],
    meetings: [
      demoMessage({
        id: "demo-msg-solstice-1",
        leadId: "demo-solstice",
        channel: "meetings",
        from: "customer",
        text: "Gong feels too heavy for a small team. Can Halvex cover call notes, transcripts, and follow-ups without another admin layer?",
        daysAgo: 1,
      }),
      demoMessage({
        id: "demo-msg-solstice-2",
        leadId: "demo-solstice",
        channel: "meetings",
        from: "rep",
        text: "Yes. Halvex keeps transcript capture, next action, and forecast signal in one workspace so reps do not switch tools.",
        daysAgo: 1,
      }),
      demoMessage({
        id: "demo-msg-solstice-3",
        leadId: "demo-solstice",
        channel: "meetings",
        from: "customer",
        text: "Good. We have budget this month, but legal needs security notes and the owner wants a Friday decision call.",
        daysAgo: 1,
      }),
    ],
    linkedin: [
      demoMessage({
        id: "demo-msg-3",
        leadId: "demo-northway",
        channel: "linkedin",
        from: "customer",
        text: "I liked your one-pager. What kind of onboarding support is included?",
        daysAgo: 2,
      }),
      demoMessage({
        id: "demo-msg-4",
        ...
        channel: "linkedin",
        from: "rep",
        text: "I can cover implementation planning and first-week training in every plan.",
        daysAgo: 2,
      }),
      demoMessage({
        id: "demo-msg-5",
        ...
        channel: "linkedin",
        from: "customer",
        text: "Need budget-aligned option for 45 seats. Can we discuss Monday?",
        daysAgo: 3,
      }),
    ],
    webchat: [
      demoMessage({
        id: "demo-msg-6",
        leadId: "demo-crescent",
        channel: "webchat",
        from: "customer",
        text: "Hi there, are you available for a quick demo?",
        daysAgo: 8,
      }),
      demoMessage({
        id: "demo-msg-7",
        ...
        channel: "webchat",
        from: "rep",
        text: "Yes, I can book a 20-min discovery window for tomorrow.",
        daysAgo: 8,
      }),
    ],
  };
  ...
  const tasks: RecoveryTask[] = [
    {
      id: "demo-task-solstice-1",
      title: "Send security notes and Gong comparison",
      description:
        "Package security posture, call intelligence workflow, and admin-light deployment plan.",
      priority: "high",
      dueAt: date(1).toISOString(),
      companyName: "Solstice Dental Group",
      personName: "Nina Frost",
    },
    {
      id: "demo-task-solstice-2",
      title: "Book owner decision call",
      description: "Confirm Friday slot with owner and legal stakeholder.",
      priority: "high",
      dueAt: date(2).toISOString(),
      companyName: "Solstice Dental Group",
      personName: "Nina Frost",
    },
    {
  ...
  const activities: RecoveryActivity[] = [
    {
      id: "demo-activity-solstice-1",
      title: "Competitor mentioned",
      body: "Buyer compared Halvex with Gong and objected to workflow weight for a small sales team.",
      type: "call_signal",
      occurredAt: date(-1).toISOString(),
      companyName: "Solstice Dental Group",
      personName: "Nina Frost",
      dealTitle: "Solstice Dental Group",
    },
    {
      id: "demo-activity-solstice-2",
      title: "Decision window captured",
      body: "Owner wants a Friday decision call after legal reviews security notes.",
      type: "intent",
      occurredAt: date(-1).toISOString(),
      companyName: "Solstice Dental Group",
      personName: "Nina Frost",
    ...
    },
    {
      ...
      body: "Customer asked for deployment timeline and support availability.",
      type: "reply",
      occurredAt: date(-2).toISOString(),
      companyName: "Northway Retail",
      personName: "Jules Moreno",
      dealTitle: "Northway Retail",
    },
    {
      id: "demo-activity-3",
      title: "Webchat bounce",
  ...
  return {
    demo: true,
    leads,
    channels: seedChannels.map((channel) => ({
      id: channel.provider,
      name: channel.name,
      connected: channel.connected,
      status: channel.status,
