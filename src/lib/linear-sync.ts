    ...
    await db
      ...
      .values(
        batch.map(issue => ({
          ...
          cycleStartsAt:  issue.cycle?.startsAt ? new Date(issue.cycle.startsAt) : null,
          cycleEndsAt:    issue.cycle?.endsAt ? new Date(issue.cycle.endsAt) : null,
      ...
      .onConflictDoUpdate({
        ...
        set: {
          ...
          cycleName:      sql`excluded.cycle_name`,
          cycleNumber:    sql`excluded.cycle_number`,
          cycleStartsAt:  sql`excluded.cycle_starts_at`,
          cycleEndsAt:    sql`excluded.cycle_ends_at`,
