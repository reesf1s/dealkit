...
export function deriveDealIntelligence(context: DealContextLike, now = new Date()) {
  ...
  const nextAction = cleanText(deal.aiNextAction)
    || cleanText(context.openTasks?.[0]?.title)
