import type { CrmLeadDto, CrmWorkspacePayload, TaskMutationInput } from '@/lib/sme-crm'

export type CrmAutomationRuleId = 'stale_deal_rescue' | 'hot_risk_proof' | 'commit_close_plan'
...
export const crmAutomationRules: CrmAutomationRule[] = [
  {
    ...
    description: 'Create a recovery task when an open deal has no recent activity.',
  ...
  },
  {
    ...
    description: 'Create proof and stakeholder tasks for hot-risk opportunities.',
  ...
  },
  {
    ...
    description: 'Create a close-plan task for high-probability forecast deals.',
...
function taskExists(workspace: CrmWorkspacePayload, lead: CrmLeadDto, title: string) {
  return workspace.tasks.some(task => task.companyName === lead.companyName && (task.title ?? '').toLowerCase() === title.toLowerCase())
}
...
function recommendation(input: {
  ...
  return {
    ...
    companyName: input.lead.companyName ?? input.lead.title ?? 'Untitled account',
...
export function buildAutomationRecommendations(workspace: CrmWorkspacePayload) {
  ...
  const staleRule = crmAutomationRules.find(rule => rule.id === 'stale_deal_rescue')!
  const riskRule = crmAutomationRules.find(rule => rule.id === 'hot_risk_proof')!
  const commitRule = crmAutomationRules.find(rule => rule.id === 'commit_close_plan')!

  for (const lead of workspace.leads.filter(isOpen)) {
    const staleTitle = `Rescue stale deal: ${lead.companyName}`
    if (daysSince(lead.latestActivityAt) >= 7 && !taskExists(workspace, lead, staleTitle)) {
    ...
    if ((lead.risk === 'hot' || Number(lead.probability ?? 0) < 45) && !taskExists(workspace, lead, riskTitle)) {
      recommendations.push(recommendation({
        rule: riskRule,
        lead,
        title: riskTitle,
        description: 'Send relevant proof, security notes, ROI context, and ask who else needs confidence before the next step.',
        reason: `${lead.risk} risk with ${lead.probability}% probability`,
      }))
    }

    const commitTitle = `Build close plan: ${lead.companyName}`
    if (Number(lead.probability ?? 0) >= 70 && !taskExists(workspace, lead, commitTitle)) {
      recommendations.push(recommendation({
        rule: commitRule,
        lead,
        title: commitTitle,
        description: 'Confirm mutual close date, buyer owner, legal/security path, and final business outcome.',
...
export function recommendationToTask(input: CrmAutomationRecommendation): TaskMutationInput {
  return {
    leadId: input.leadId,
    title: input.title,
