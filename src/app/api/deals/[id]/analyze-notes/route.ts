...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [deal] = await db.select().from(dealLogs).where(and(eq(dealLogs.id, id), eq(dealLogs.userId, userId))).limit(1)
    ...
    const msg = await anthropic.messages.create({
      ...
      messages: [{
        role: 'user',
        content: `You are analyzing B2B sales meeting notes. Extract structured information and return ONLY valid JSON, no markdown.
...
conversionScore: 0-100 based on signals (budget confirmed=+20, timeline=+15, champion=+15, competition mentioned=-10, objections=-10, etc)
priority for gaps: critical | high | medium | low`
      }],
    })
    ...
    // Update deal
    const todos = (parsed.todos ?? []).map((t: any) => ({
    ...
    for (const gap of (parsed.productGaps ?? [])) {
      ...
      const [existing] = await db.select().from(productGaps)
        .where(and(eq(productGaps.userId, userId), eq(productGaps.title, gap.title))).limit(1)
      if (existing) {
        const [updated] = await db.update(productGaps).set({
          frequency: (existing.frequency ?? 1) + 1,
          sourceDeals: [...((existing.sourceDeals as string[]) ?? []), id],
          updatedAt: new Date(),
        }).where(eq(productGaps.id, existing.id)).returning()
        createdGaps.push(updated)
      } else {
        const [created] = await db.insert(productGaps).values({
          userId, title: gap.title, description: gap.description ?? '',
          priority: gap.priority ?? 'medium', frequency: 1,
          sourceDeals: [id], status: 'open',
