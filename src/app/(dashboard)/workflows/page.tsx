...
import {
  Zap, Plus, CheckCircle2, Clock, GitBranch,
...
const DEFAULT_TEMPLATES: Workflow[] = [
  {
    ...
    title: 'Daily linked issues briefing',
    trigger: 'Every day at 8am',
    action: 'Show in Today tab: @linear issues in @cyclecurrent linked to deals with their status',
    active: true,
    isTemplate: true,
    tokens: [
      'Show in Today tab: ',
  ...
  {
    id: 'deal-risk-alerts',
    title: 'Deal health drop alerts',
    trigger: 'When a deal score drops > 10 points',
    action: 'DM me on @slack with the risk factors and suggested action',
    active: true,
    isTemplate: true,
    tokens: [
      'DM me on ',
      { type: 'mention', key: 'slack' },
      ' with the risk factors and suggested action',
    ],
  },
  {
    id: 'release-email-reminder',
    title: 'Shipped issue release email',
    trigger: 'When a Linear issue linked to a deal is completed',
    action: 'Draft release email and DM me on @slack to review and send',
    active: false,
    isTemplate: true,
    tokens: [
      'Draft release email and DM me on ',
      { type: 'mention', key: 'slack' },
      ' to review and send',
    ],
  },
...
function MentionInput({
  ...
  // Render the value with coloured chips overlay
  ...
  return (
    <div style={{ position: 'relative' }}>
      {/* Preview chips overlay (non-interactive, just visual) */}
      <div style={{
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: idx === selectedIdx ? 'rgba(255,255,255,0.07)' : 'transparent',
                }}
              >
                <span style={{
                  background: chip.bg, color: chip.color,
...
function WorkflowCard({
  ...
  const renderTokens = (tokens?: WorkflowToken[], fallback?: string) => {
    if (!tokens) return <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.50)' }}>{fallback}</span>
    return (
      <span style={{ fontSize: '12px', lineHeight: 1.6 }}>
        {tokens.map((t, i) => {
          if (typeof t === 'string') {
            return <span key={i} style={{ color: 'rgba(255,255,255,0.55)' }}>{t}</span>
          }
  ...
  return (
    <div style={{
      ...
      border: `1px solid ${workflow.active ? 'rgba(129,140,248,0.20)' : 'rgba(255,255,255,0.07)'}`,
      borderRadius: '12px',
      padding: '16px 20px',
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '12px' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
            <Clock size={10} style={{ color: 'rgba(255,255,255,0.28)', flexShrink: 0 }} />
            <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.38)' }}>{workflow.trigger}</span>
          </div>

          {/* Action */}
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '6px' }}>
            <ArrowRight size={10} style={{ color: 'rgba(255,255,255,0.28)', flexShrink: 0, marginTop: '3px' }} />
        ...
        <button
          ...
          style={{
            ...
            background: workflow.active ? '#818cf8' : 'rgba(255,255,255,0.12)',
    ...
    <div style={{ maxWidth: '760px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px' }}>
          ...
          <h1 style={{ fontSize: '24px', fontWeight: 600, color: 'rgba(255,255,255,0.90)', letterSpacing: '-0.02em', margin: 0 }}>
            Workflows
          </h1>
          <span style={{
        ...
        <p style={{ fontSize: '13px', color: 'rgba(255,255,255,0.38)', margin: 0 }}>
          Automate actions across your integrations. Use <strong style={{ color: '#818cf8' }}>@</strong> to reference integrations and data sources.
        </p>
      </div>
      ...
      {!showNewForm ? (
        <button
          ...
          style={{
            ...
            color: 'rgba(255,255,255,0.40)', fontSize: '12px', fontWeight: 500,
        ...
        <div style={{
          ...
          <div style={{ marginBottom: '10px' }}>
            <label style={{ fontSize: '11px', fontWeight: 600, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: '5px' }}>
              Trigger
            </label>
            <MentionInput
              value={newTrigger}
          ...
          <div style={{ marginBottom: '14px' }}>
            <label style={{ fontSize: '11px', fontWeight: 600, color: 'rgba(255,255,255,0.45)', textTransform: 'uppercase', letterSpacing: '0.06em', display: 'block', marginBottom: '5px' }}>
              Action — type @ to reference integrations
            </label>
            <MentionInput
              value={newAction}
          ...
          {recentActions.map((action: any) => {
            ...
            return (
              <div key={action.id} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                ...
                <span style={{ flex: 1, fontSize: '12px', color: 'rgba(255,255,255,0.60)' }}>
                  {action.label}
                  {action.dealName && <span style={{ color: 'rgba(255,255,255,0.30)' }}> · {action.dealName}</span>}
                </span>
                <span style={{ fontSize: '10px', color: 'rgba(255,255,255,0.22)', flexShrink: 0 }}>{timeAgo}</span>
              </div>
            )
          })}
        </div>
