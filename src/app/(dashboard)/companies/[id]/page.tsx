...
export default function CompanyPage() {
  ...
  if (!company) return <CrmPage><CrmEmpty title="Company not found" /></CrmPage>
  ...
  return (
    <CrmPage wide>
      <ObjectWorkspaceHeader
        object="Company"
        title={company.name}
        actions={<><CrmButton href="/deals?quick=deal" tone="primary"><Plus size={16} /> Add deal</CrmButton><CrmButton onClick={() => window.dispatchEvent(new CustomEvent('openHalvexAssistant', { detail: { query: `Give me the account read for ${company.name}: open deals, people, tasks, notes, risk, and next action.` } }))}><Bot size={16} /> Analyse</CrmButton></>}
        stats={<>
      ...
      <div className="crm-record-layout">
        <main className="crm-record-main">
          <CrmPanel>
            <CrmSectionHeader title="Account details" />
            <CompanyDetailsForm company={company} peopleCount={people.length} taskCount={tasks.length} onSaved={mutateCompanies} />
          </CrmPanel>
          <CrmPanel>
            <CrmSectionHeader title="People" action={<CrmButton href="/people?quick=person"><Plus size={16} /> Add person</CrmButton>} />
