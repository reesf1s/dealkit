  ...
  async function handleGenerateBattlecard(competitorId: string) {
    try {
      const res = await fetch('/api/collateral/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'battlecard', sourceCompetitorId: competitorId }),
      })
    ...
    <div style={{ padding: '32px', maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '28px' }}>
        <div>
          ...
          <p style={{ fontSize: '13px', color: '#888', margin: 0 }}>
            {competitors.length} tracked • Battlecards powered by AI
          </p>
        </div>
      ...
      <Dialog.Root open={addOpen} onOpenChange={setAddOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', zIndex: 501, width: '100%', maxWidth: '640px', maxHeight: '90vh', overflowY: 'auto', backgroundColor: '#141414', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '24px', boxShadow: '0 16px 48px rgba(0,0,0,0.8)', outline: 'none' }}>
