...
export const pillSurfaceClass =
  "rounded-2xl border border-white/[0.08] bg-[#12111a]/90 shadow-[0_18px_60px_rgba(0,0,0,0.16),inset_0_1px_0_rgba(255,255,255,0.04)]";
export const pillInsetClass =
  "rounded-xl border border-white/[0.07] bg-black/20";
export const tableRowClass =
  "rounded-xl border border-white/[0.07] bg-white/[0.035] px-4 py-3 text-sm transition hover:bg-white/[0.065]";
      ...
      <div className="mx-auto flex h-14 max-w-7xl items-center justify-between gap-3 rounded-full border border-white/10 bg-black/55 px-3 shadow-[0_18px_70px_rgba(0,0,0,0.35)] backdrop-blur-2xl">
        ...
        <div className="flex items-center gap-2">
          <Button
            asChild
            variant="ghost"
            className="h-10 rounded-full px-4 text-sm text-zinc-200 hover:bg-white/[0.08] hover:text-white"
          >
            <Link href="/sign-in">Sign in</Link>
          </Button>
          <Button
            asChild
            className="h-10 rounded-full bg-white px-5 text-sm text-black hover:bg-zinc-200"
          >
            <Link href="/sign-up">
              Start free
...
export function PublicBackdrop({
  ...
  return (
    <main
      className={cn(
        "relative min-h-screen overflow-hidden bg-[#050607] text-white",
        className,
      )}
    >
  ...
  return (
    <div className="grid gap-2 rounded-[30px] border border-white/10 bg-black/42 p-2 backdrop-blur-xl md:grid-cols-3">
      {items.map((item) => (
        <div
          key={item.label}
          className="rounded-full border border-white/10 bg-white/[0.045] px-5 py-4"
        >
          <div className="flex items-center justify-between gap-4">
            <span className="text-xs text-zinc-400">{item.label}</span>
            <strong className="font-title text-2xl font-semibold text-white">
              {item.value}
            </strong>
          </div>
          ...
          <section className="hidden lg:block">
            ...
            <div className="mt-8 grid max-w-xl gap-2">
              {[
                ...
                "Deal risk and next step rows",
                "AI drafts grounded in CRM context",
              ].map((item) => (
                <div
                  key={item}
                  className={cn(tableRowClass, "flex items-center gap-3")}
                >
          ...
          <section className="mx-auto grid w-full max-w-[520px] gap-5">
            ...
            <div className={cn(pillSurfaceClass, "p-3")}>
              <div className="px-4 py-5 text-center">
                ...
                <h2 className="mt-5 font-title text-3xl font-semibold">
                  {title}
                </h2>
              ...
              <div className="min-h-[430px] px-3 pb-3">{children}</div>
            </div>
...
export const authAppearance = {
  ...
  elements: {
    ...
    socialButtonsBlockButton:
      "rounded-full border-white/10 bg-white/[0.04] text-zinc-100 hover:bg-white/[0.08]",
    dividerLine: "bg-white/10",
    dividerText: "text-zinc-500",
    formFieldLabel: "text-zinc-200",
    formFieldInput:
      "rounded-full border-white/10 bg-black/25 text-zinc-100 placeholder:text-zinc-500",
