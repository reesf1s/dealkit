  ...
  for (const messages of Object.values(workspace.messages)) {
    for (const message of messages) {
      const lead = workspace.leads.find(
        (candidate) => candidate.id === message.leadId,
      );
      results.push({
...
export default function WorkspaceCommandBar() {
  ...
  const [commandMode, setCommandMode] = useState<"deal" | "task" | "activity">(
    "deal",
  );
  ...
  useEffect(() => {
    ...
    if (firstLeadId && !taskForm.leadId)
      setTaskForm((current) => ({ ...current, leadId: firstLeadId }));
    if (firstLeadId && !activityForm.leadId)
      setActivityForm((current) => ({ ...current, leadId: firstLeadId }));
    ...
    try {
      await apiJson("/api/crm/tasks", {
        ...
        body: JSON.stringify({
          ...
          description:
            taskForm.description.trim() || "Created from the command bar.",
          priority: taskForm.priority,
          dueAt: taskForm.dueAt
            ? new Date(`${taskForm.dueAt}T10:00:00.000Z`).toISOString()
            : undefined,
      ...
      setNotice(
        `Task created${lead?.companyName ? ` for ${lead.companyName}` : ""}`,
      );
      setTaskForm({
        leadId: lead?.id ?? workspace?.leads[0]?.id ?? "",
        title: "",
        description: "",
        priority: "medium",
        dueAt: defaultDueDateInput(),
  ...
  async function handleLogActivity() {
    const lead = workspace?.leads.find(
      (item) => item.id === activityForm.leadId,
    );
    ...
    try {
      ...
      setNotice(
        `Activity logged${lead?.companyName ? ` for ${lead.companyName}` : ""}`,
      );
      setActivityForm({
        leadId: lead?.id ?? workspace?.leads[0]?.id ?? "",
        type: "note",
  ...
  return (
    <>
      <div className="relative min-w-0 flex-1">
        ...
        {focused ? (
          <div
            className={cn(
              pillSurfaceClass,
              "absolute left-0 right-0 top-12 z-50 max-h-[520px] overflow-hidden bg-[#090b0d] p-2 shadow-2xl",
            )}
          >
            <div className="grid gap-1">
              {filtered.map((item) => (
                <Link
                  ...
                  className={cn(
                    pillInsetClass,
                    "grid gap-2 p-3 transition hover:bg-white/[0.07]",
                  )}
                >
                  <span className="flex min-w-0 items-center justify-between gap-3">
                    <span className="flex min-w-0 items-center gap-3">
                      <span className="grid size-8 place-items-center rounded-full bg-white/[0.06] text-zinc-300">
      ...
      <Button
        ...
        aria-label="New"
        className="inline-flex size-9 rounded-lg bg-violet-500 px-0 text-xs text-white hover:bg-violet-400 md:w-auto md:px-3"
      >
      ...
      <Sheet open={createOpen} onOpenChange={setCreateOpen}>
        <SheetContent
          side="right"
          className="w-full overflow-y-auto border-white/10 bg-[#080a0d] p-0 text-zinc-100 sm:max-w-2xl"
        >
          <SheetHeader className="border-b border-white/10 p-6">
            <SheetTitle className="font-title text-2xl text-white">
              Create
            </SheetTitle>
            <SheetDescription className="text-zinc-400">
              Capture the minimum now. Add detail only when it helps the sale.
            </SheetDescription>
          </SheetHeader>
          <div className="grid gap-4 p-6">
            ...
            {error ? (
              <div className="rounded-full border border-red-300/20 bg-red-300/10 px-4 py-2 text-sm text-red-100">
                {error}
              </div>
            ) : null}
            ...
            <Tabs
              value={commandMode}
              onValueChange={(value) =>
                setCommandMode(value as typeof commandMode)
              }
              ...
              <TabsContent value="deal" className="mt-0 grid gap-4">
                  ...
                  <Label className="grid gap-2 text-xs text-zinc-500">
                    ...
                    <Input
                      ...
                      onChange={(event) =>
                        setForm({
                          ...form,
                          primaryPersonName: event.target.value,
                        })
                      }
                      className="rounded-full border-white/10 bg-black/20 text-white"
                    />
                  ...
                  <div className="mt-4 grid gap-4 md:grid-cols-2">
                    ...
                    <Label className="grid gap-2 text-xs text-zinc-500">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setForm({
                            ...form,
                            channel: event.target.value as ChannelId,
                          })
                        }
                        className="h-9 rounded-full border border-white/10 bg-black/40 px-3 text-sm text-white outline-none"
                      >
                    ...
                    <Label className="grid gap-2 text-xs text-zinc-500">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setForm({
                            ...form,
                            risk: event.target.value as CrmLeadDto["risk"],
                          })
                        }
                        className="h-9 rounded-full border border-white/10 bg-black/40 px-3 text-sm text-white outline-none"
                      >
                    ...
                    <Label className="grid gap-2 text-xs text-zinc-500 md:col-span-2">
                      ...
                      <Textarea
                        ...
                        className="min-h-24 rounded-xl border-white/10 bg-black/20 text-white"
                      />
                    </Label>
                  </div>
                </details>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Button
                    asChild
                    variant="outline"
                    className="rounded-full border-white/10 bg-white/[0.04] text-zinc-100"
                  >
              ...
              <TabsContent value="task" className="mt-0 grid gap-4">
                <Label className="grid gap-2 text-xs text-zinc-500">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setTaskForm({ ...taskForm, leadId: event.target.value })
                    }
                    className="h-10 rounded-full border border-white/10 bg-black/40 px-3 text-sm text-white outline-none"
                  >
                ...
                <Label className="grid gap-2 text-xs text-zinc-500">
                  ...
                  <Textarea
                    ...
                    className="min-h-28 rounded-[22px] border-white/10 bg-black/20 text-white"
                  />
                </Label>
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    disabled={busy || !workspace?.leads.length}
                    className="rounded-full bg-white text-black hover:bg-zinc-200"
                  >
              ...
              <TabsContent value="activity" className="mt-0 grid gap-4">
                ...
                <Label className="grid gap-2 text-xs text-zinc-500">
                  ...
                  <Textarea
                    ...
                    className="min-h-36 rounded-[22px] border-white/10 bg-black/20 text-white"
                  />
                </Label>
                <div className="flex flex-wrap gap-2">
                  <Button
