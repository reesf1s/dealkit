...
export default function WorkspaceCallReview() {
  ...
  const [busy, setBusy] = useState<"load" | "analyze" | "commit" | null>(
    "load",
  );
  ...
  const selectedLead = useMemo(
    () => workspace?.leads.find((lead) => lead.id === leadId) ?? null,
    [workspace, leadId],
  );
  ...
  return (
    <div className="grid gap-4 text-zinc-100">
      <ScreenHeader
        ...
        title="Call review"
        description="Enterprise-grade call analysis for lean SaaS teams: capture objections, decision signals, risks, and next steps directly in the CRM."
        action={
          <Button
            type="button"
            variant="outline"
      ...
      <section className="grid gap-4 md:grid-cols-4">
        {[
          {
            label: "Selected deal",
            value: selectedLead?.companyName ?? "None",
            detail: selectedLead
              ? `${money(Number(selectedLead.valueAmount ?? 0))} pipeline`
              : "Choose a deal",
            icon: PhoneCall,
          },
          {
            label: "Sentiment",
            value: analysis?.sentiment ?? "Pending",
            detail: "Derived from transcript signals",
            icon: Brain,
          },
          {
            label: "Risks",
            value: `${analysis?.risks.filter((item) => !item.startsWith("No explicit")).length ?? 0}`,
            detail: "Commercial and process risks",
            icon: ShieldAlert,
          },
          {
            label: "Next steps",
            value: `${analysis?.nextSteps.length ?? 0}`,
            detail: "Actionable follow-up items",
            icon: ClipboardList,
          },
        ].map((item) => {
          const Icon = item.icon;
          return (
            <Card key={item.label} className={cn(surface, "gap-0 py-0")}>
              <CardContent className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-xs text-zinc-500">{item.label}</p>
                    <p className="mt-2 truncate text-2xl font-semibold tracking-normal text-white">
                      {busy === "load" ? "..." : item.value}
                    </p>
                  </div>
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        <Card className={cn(surface, "gap-0 py-0")}>
          <CardHeader>
            ...
            <CardDescription>
              Select the deal, paste a call transcript, then preview or commit
              the review.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4">
            ...
            <Textarea
              ...
              className="min-h-[280px] rounded-[24px] border-white/10 bg-black/20 font-mono text-xs leading-6 text-white placeholder:text-zinc-600"
            />
        ...
        <Card className={cn(surface, "gap-0 py-0")}>
          <CardHeader>
            ...
            <CardDescription>
              What will be written back to the CRM when committed.
            </CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4">
            {analysis ? (
              <>
                <div className={cn(inset, "p-4")}>
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge
                      variant="outline"
                      className={sentimentTone(analysis.sentiment)}
                    >
                      {analysis.sentiment}
                    </Badge>
                ...
                {[
                  ...
                  },
                  {
                    ...
                    items: analysis.decisionSignals,
                    icon: CheckCircle2,
                  },
                  {
                    title: "Next steps",
                  ...
                  return (
                    <div key={section.title} className={cn(inset, "p-4")}>
                ...
                <div className={cn(inset, "p-4")}>
                  <p className="font-semibold text-white">
                    {analysis.recommendedTask.title}
                  </p>
                  <p className="mt-2 whitespace-pre-line text-sm leading-6 text-zinc-400">
                    {analysis.recommendedTask.description}
                  </p>
                </div>
              </>
            ) : (
              <div className="py-20 text-center text-sm text-zinc-500">
                Analyze a transcript to see call intelligence here.
              </div>
            )}
          </CardContent>
        </Card>
